angular.module('myApp').controller('Form5Controller', ['$http', '$scope', '$routeParams', '$location', '$q', '$timeout', 'NavigationService', 'FamiliaService', 'DiagnosticoSocialService', 'UserErrorService', function ($http, $scope, $routeParams, $location, $q, $timeout, NavigationService, FamiliaService, DiagnosticoSocialService, UserErrorService) {
	$scope.isDirty = false;
	...
	function calculate() {
		...
		if (indicadores === 0) {
			...
			$scope.form.valoracion = 'A'; //Adecuado		- 0 marcados
		}
		else if (q[5]) {
			$scope.form.valoracion = 'MD'; //Muy deficitario	-  marcado 6 indicador
		}
		else if (indicadores == 1) {
			$scope.form.valoracion = 'A'; //Adecuado		- 1 marcado entre los 5 primeros
		}
		else if (indicadores == 2) {
			$scope.form.valoracion = 'D'; //Deficitario		- 2 marcado entre los 5 primeros
		}
		else {
	...
	function saveState() {
		...
		//saves state async
		DiagnosticoSocialService.prepareForSaveArray($scope.diagnostico, 'f5', $scope.form, 'q');
	...
	function loadDiagnostico(httpResponse) {
		...
		FamiliaService.getToEdit($scope.diagnostico.familiaId)
			.then(loadFamilia,
			 	  errorHandler);
		$scope.form = DiagnosticoSocialService.loadFormAsBoolArray('f5', $scope.diagnostico, 'q', 6, null);		

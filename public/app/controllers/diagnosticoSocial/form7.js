angular.module('myApp').controller('Form7Controller', ['$http', '$scope', '$routeParams', '$location', '$q', '$timeout', 'NavigationService', 'FamiliaService', 'DiagnosticoSocialService', 'UserErrorService', function ($http, $scope, $routeParams, $location, $q, $timeout, NavigationService, FamiliaService, DiagnosticoSocialService, UserErrorService) {
	$scope.isDirty = false;
	...
	function saveState() {
		...
		//saves state async
		DiagnosticoSocialService.prepareForSaveArray($scope.diagnostico, 'f7', $scope.form, 'q');
	...
	function loadDiagnostico(httpResponse) {
		...
		FamiliaService.getToEdit($scope.diagnostico.familiaId)
			.then(loadFamilia,
			 	  errorHandler);
		$scope.form = DiagnosticoSocialService.loadFormAsBoolArray('f7', $scope.diagnostico, 'q', 7, null);		

angular.module('myApp').controller('ImportController', ['$scope', '$routeParams', '$location', 'NavigationService', 'ImportService', function ($scope, $routeParams, $location, NavigationService, ImportService) {
	...
	*/
	...
	$scope.cancel = function () {
		...
		var url = NavigationService.pop().url || '/'+$scope.className+'/list';
		$location.path(url);
	};
	...
	$scope.importPreview = function () {
		...
		var data = ImportService.selectDataToImport($scope.previewData, $scope.className, $scope.currentWorkSheet);
		$scope.previewData = data;
	};

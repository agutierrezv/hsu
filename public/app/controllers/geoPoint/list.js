angular.module('myApp').controller('ListGeoPointController', ['$http', '$scope', '$location', '$q', '$timeout', 'NavigationService', 'GeoPointService', function ($http, $scope, $location, $q, $timeout, NavigationService, GeoPointService) {
	$scope.dataList = [];
	...
	$scope.add = function () {
		$location.path('/geoPoint/add');
	};

	$scope.edit = function (obj) {
		$location.path('/geoPoint/edit/' + obj._id);
	};
	
	$scope.delete = function (obj) {
		$location.path('/geoPoint/delete/' + obj._id);
	};
	...
	$scope.deleteSelected = function() {
		...
		GeoPointService.deleteMany(ids)
		//.catch(function(err) {
		//})
		.finally(function(f) {
			$timeout(function(){
	...
	$scope.importData = function () {
		...
		$location.path('/import/geoPoint');		
	};

	$scope.exportAsCsv = function () { 
		$scope.dataReceived = false;	
		GeoPointService.getFileAsCsv()
		.then(function(httpResponse) {
			sendFile(httpResponse, "geoPoints.csv", "text/csv");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXml = function () { 
		$scope.dataReceived = false;	
		GeoPointService.getFileAsXml().then(function(httpResponse) {
			sendFile(httpResponse, "geoPoints.xml", "text/xml");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXlsx = function () {
		$scope.dataReceived = false;	
		GeoPointService.getFileAsXlsx().then(function(httpResponse) {
			sendFile(httpResponse, "geoPoints.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	function sendFile(httpResponse, fileName, mime) {
		...
		var filename = headers["x-filename"] || fileName || "geoPoints.csv";

angular.module('myApp').controller('ListProfesionalController', ['$http', '$scope', '$location', '$q', '$timeout', 'NavigationService', 'ProfesionalService', function ($http, $scope, $location, $q, $timeout, NavigationService, ProfesionalService) {
	$scope.dataList = [];
	...
	$scope.add = function () {
		$location.path('/profesional/add');
	};

	$scope.edit = function (obj) {
		$location.path('/profesional/edit/' + obj._id);
	};
	
	$scope.delete = function (obj) {
		$location.path('/profesional/delete/' + obj._id);
	};
	...
	$scope.deleteSelected = function() {
		...
		ProfesionalService.deleteMany(ids)
		//.catch(function(err) {
		//})
		.finally(function(f) {
			$timeout(function(){
	...
	$scope.importData = function () {
		...
		$location.path('/import/profesional');		
	};

	$scope.exportAsCsv = function () { 
		$scope.dataReceived = false;	
		ProfesionalService.getFileAsCsv()
		.then(function(httpResponse) {
			sendFile(httpResponse, "profesionals.csv", "text/csv");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXml = function () { 
		$scope.dataReceived = false;	
		ProfesionalService.getFileAsXml().then(function(httpResponse) {
			sendFile(httpResponse, "profesionals.xml", "text/xml");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXlsx = function () {
		$scope.dataReceived = false;	
		ProfesionalService.getFileAsXlsx().then(function(httpResponse) {
			sendFile(httpResponse, "profesionals.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	function sendFile(httpResponse, fileName, mime) {
		...
		var filename = headers["x-filename"] || fileName || "profesionals.csv";

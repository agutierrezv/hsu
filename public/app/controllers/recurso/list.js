angular.module('myApp').controller('ListRecursoController', ['$http', '$scope', '$location', '$q', '$timeout', 'NavigationService', 'RecursoService', function ($http, $scope, $location, $q, $timeout, NavigationService, RecursoService) {
	$scope.dataList = [];
	...
	$scope.add = function () {
		$location.path('/recurso/add');
	};

	$scope.edit = function (obj) {
		$location.path('/recurso/edit/' + obj._id);
	};
	
	$scope.delete = function (obj) {
		$location.path('/recurso/delete/' + obj._id);
	};
	...
	$scope.importData = function () {
		...
		$location.path('/import/recurso');		
	};

	$scope.exportAsCsv = function () { 
		$scope.dataReceived = false;	
		RecursoService.getFileAsCsv()
		.then(function(httpResponse) {
			sendFile(httpResponse, "recursos.csv", "text/csv");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXml = function () { 
		$scope.dataReceived = false;	
		RecursoService.getFileAsXml().then(function(httpResponse) {
			sendFile(httpResponse, "recursos.xml", "text/xml");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXlsx = function () {
		$scope.dataReceived = false;	
		RecursoService.getFileAsXlsx().then(function(httpResponse) {
			sendFile(httpResponse, "recursos.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	function sendFile(httpResponse, fileName, mime) {
		...
		var filename = headers["x-filename"] || fileName || "recursos.csv";

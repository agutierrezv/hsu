angular.module('myApp').controller('ListEntidadController', ['$http', '$scope', '$location', '$q', '$timeout', 'NavigationService', 'EntidadService', function($http, $scope, $location, $q, $timeout, NavigationService, EntidadService) {
	$scope.dataList = [];
	...
	$scope.add = function () {
		$location.path('/entidad/add');
	};

	$scope.edit = function (obj) {
		$location.path('/entidad/edit/' + obj._id);
	};
	
	$scope.delete = function (obj) {
		$location.path('/entidad/delete/' + obj._id);
	};
	...
	$scope.importData = function () {
		...
		$location.path('/import/entidad');		
	};

	$scope.exportAsCsv = function () { 
		$scope.dataReceived = false;	
		EntidadService.getFileAsCsv()
		.then(function(httpResponse) {
			sendFile(httpResponse, "entidads.csv", "text/csv");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXml = function () { 
		$scope.dataReceived = false;	
		EntidadService.getFileAsXml().then(function(httpResponse) {
			sendFile(httpResponse, "entidads.xml", "text/xml");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXlsx = function () {
		$scope.dataReceived = false;	
		EntidadService.getFileAsXlsx().then(function(httpResponse) {
			sendFile(httpResponse, "entidads.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	function sendFile(httpResponse, fileName, mime) {
		...
		var filename = headers["x-filename"] || fileName || "entidads.csv";

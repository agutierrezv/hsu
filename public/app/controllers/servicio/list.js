angular.module('myApp').controller('ListServicioController', ['$http', '$scope', '$location', '$q', '$timeout', 'NavigationService', 'ServicioService', function ($http, $scope, $location, $q, $timeout, NavigationService, ServicioService) {
	$scope.dataList = [];
	...
	$scope.add = function () {
		$location.path('/servicio/add');
	};

	$scope.edit = function (obj) {
		$location.path('/servicio/edit/' + obj._id);
	};
	
	$scope.delete = function (obj) {
		$location.path('/servicio/delete/' + obj._id);
	};
	...
	$scope.deleteSelected = function() {
		...
		ServicioService.deleteMany(ids)
		//.catch(function(err) {
		//})
		.finally(function(f) {
			$timeout(function(){
	...
	$scope.importData = function () {
		...
		$location.path('/import/servicio');		
	};

	$scope.exportAsCsv = function () { 
		$scope.dataReceived = false;	
		ServicioService.getFileAsCsv()
		.then(function(httpResponse) {
			sendFile(httpResponse, "servicios.csv", "text/csv");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXml = function () { 
		$scope.dataReceived = false;	
		ServicioService.getFileAsXml().then(function(httpResponse) {
			sendFile(httpResponse, "servicios.xml", "text/xml");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	$scope.exportAsXlsx = function () {
		$scope.dataReceived = false;	
		ServicioService.getFileAsXlsx().then(function(httpResponse) {
			sendFile(httpResponse, "servicios.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		})
		.finally(function() {
			$scope.dataReceived = true;
		});
	...
	function sendFile(httpResponse, fileName, mime) {
		...
		var filename = headers["x-filename"] || fileName || "servicios.csv";

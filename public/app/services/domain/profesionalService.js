angular.module('myApp').service('ProfesionalService', ['$http', '$q', 'UrlService', 'baseApi', function ($http, $q, UrlService, baseApi) {

	var ProfesionalService = {};
	...
	// Getting the profesional API documentation URL and a promise to know when it's loaded.
	docUrlReceived = UrlService.profesionalUrl.then(function (url) {
		docUrl = url;
	});

	// When the documentation is received, getting the profesionals resource URL and a promise to know when it's loaded.
...
//Process each property
		...
		clause = addStringLike('tipoIndentificador', searchText);
		...
		clause = addStringLike('prefijo', searchText);
		...
		clause = addStringLike('telefono', searchText);
		...
		clause = addBooleanLike('acreditado', searchText);
		...
		clause = addStringLike('nivel', searchText);

		if (clause != null){
			clauses.push(clause);
		}

		clause = addStringLike('perfil', searchText);
	...
	ProfesionalService.getCount =  safeCall(function (opts) {
		opts = opts || {};
		opts.count = true;		
	...
	ProfesionalService.getList = safeCall(function (opts) {
		opts = opts || {};
		var q = buildBaucisQuery(opts);
		return $http.get(resourceUrl + q);
	});

	ProfesionalService.getListAsCsv = safeCall(function () {
		return $http({
			method: 'GET', 
			url: resourceUrl, 
	...
	ProfesionalService.getFileAsCsv = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	ProfesionalService.getFileAsXml = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	ProfesionalService.getFileAsXlsx = safeCall(function () {
		return $http({
			method: 'GET', 
	...
	ProfesionalService.getToEdit = safeCall(function (id) {
		return ProfesionalService.get(resourceUrl + '/' + id );
	});

	ProfesionalService.get = function (link) {
		return $http.get(link);
	};

	ProfesionalService.add = safeCall(function (item) {
		return $http.post(resourceUrl, JSON.stringify(item));
	});

	ProfesionalService.update = function (item) {
		return $http.put(resourceUrl + '/' + item._id, JSON.stringify(item));
	};

	ProfesionalService.delete = safeCall(function (id) {
		return $http.delete(resourceUrl + '/' + id);
	});

	ProfesionalService.deleteMany = safeCall(function (ids) {
		var msg = { 
	...
	ProfesionalService.deleteAll = safeCall(function (opts) {
		var msg = { 
			'className' : 'profesional',

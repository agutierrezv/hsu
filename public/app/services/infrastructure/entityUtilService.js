angular.module('myApp').service('EntityUtilService', ['$timeout', function ($timeout) {
	...
	//Export functions -------------------------------------------
	function exportAsCsv(service, label, scope) { 
		scope.dataReceived = false;	

		service.getFileAsCsv()
	...
	function exportAsXml(service, label, scope) { 
		scope.dataReceived = false;	
		service.getFileAsXml().then(function(httpResponse) {
	...
	function exportAsXlsx(service, label, scope) {
		scope.dataReceived = false;	
		service.getFileAsXlsx().then(function(httpResponse) {
	...
	EntityUtilService.exportAsFormat = function (format, service, label, scope) { 
		if (format === 'xml') {
			exportAsCsv(service, label, scope);
		} else if (format === 'csv') {
			exportAsCsv(service, label, scope);
		} else if (format === 'xlsx') {
			exportAsXml(service, label, scope);			
		}
	};
	...
	//Delete operations -----
	EntityUtilService.deleteAll = function (service, searchCriteria, refreshCallback) {		
		service.deleteAll(searchCriteria)
			.finally(function(f) {
				$timeout(function() {
					if (refreshCallback) {
						refreshCallback();
